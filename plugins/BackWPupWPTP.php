...
class BackWPupWPTP extends TelegramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="backwpup_send_log_notification"><?php _e('Notification for send log', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="backwpup_send_log_notification"
                              name="backwpup_send_log_notification" <?php checked($this->get_option('backwpup_send_log_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <?php
    ...
    {
        if (stripos($args['subject'], 'BackWPup') === false) return $args;
        ...
        $text = apply_filters('telegrambot_backwpup_send_log_notification_text', $text, $args);
