...
class VisualFormBuilderWPTP extends TgramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="visualformbuilder_new_message_notification"><?php _e('Notification for new message', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="visualformbuilder_new_message_notification"
                              name="visualformbuilder_new_message_notification" <?php checked($this->get_option('visualformbuilder_new_message_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <tr>
            <td>
                <label for="visualformbuilder_forms_select"><?php _e('Notification from this forms', $this->plugin_key) ?></label>
            </td>
            <td>
                <?php
                self::forms_select(
    ...
    function form_submit($form_id)
    {
        ...
        if ($entry) {
            ...
            if (is_serialized($data)) {
                ...
                $valid_type = array('text', 'textarea', 'radio', 'checkbox',
