...
class WPSMSWPTP extends TelegramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="wpsms_add_subscriber_notification"><?php _e('Notification for new subscriber', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="wpsms_add_subscriber_notification"
                              name="wpsms_add_subscriber_notification" <?php checked($this->get_option('wpsms_add_subscriber_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <?php
