...
class NewsletterWPTP extends TgramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="newsletter_plugin_new_subscriber_notification"><?php _e('Notification for new subscriber', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="newsletter_plugin_new_subscriber_notification"
                              name="newsletter_plugin_new_subscriber_notification" <?php checked($this->get_option('newsletter_plugin_new_subscriber_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <tr>
            <td>
                <label for="newsletter_plugin_unsubscribe_notification"><?php _e('Notification for unsubscribe', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="newsletter_plugin_unsubscribe_notification"
                              name="newsletter_plugin_unsubscribe_notification" <?php checked($this->get_option('newsletter_plugin_unsubscribe_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <?php
    ...
    {
        if ($user == null || !is_object($user) || !class_exists('TNP_User')) return;
        ...
        $title = $status == 'subscribe' ? __('New subscriber for newsletter', $this->plugin_key) : __('Unsubscribe from newsletter', $this->plugin_key);
