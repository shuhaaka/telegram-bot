...
class WPCerberSecurityWPTP extends TelegramBot
{
        ...
        <tr>
            ...
            <td>
                ...
                foreach ($this->events as $key => $label) {
                    echo '<label>
                            <input type="checkbox" value="' . $key . '" name="wpcerbersecurity_events[]" ' . checked(in_array($key, $events), true, false) . '> ' . $label . '
                            </label><br>';
                }
                ?>
            </td>
    ...
    {
        $events = $this->get_option('wpcerbersecurity_events', []);
        if (!is_array($params) || !in_array($params['type'], $events)) return;
