...
class CalderaFormsWPTP extends TelegramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="calderaforms_new_message_notification"><?php _e('Notification for new message', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="calderaforms_new_message_notification"
                              name="calderaforms_new_message_notification" <?php checked($this->get_option('calderaforms_new_message_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <tr>
            <td>
                <label for="calderaforms_forms_select"><?php _e('Notification from this forms', $this->plugin_key) ?></label>
            </td>
            <td>
                <?php
                self::forms_select(
    ...
    function form_submit($form, $referrer, $process_id, $entryid)
    {
        ...
        $text = apply_filters('telegrambot_calderaforms_message_notification_text', $text, $form);
