...
class EverestFormsWPTP extends TelegramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="everestforms_new_message_notification"><?php _e('Notification for new message', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="everestforms_new_message_notification"
                              name="everestforms_new_message_notification" <?php checked($this->get_option('everestforms_new_message_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
            </td>
        </tr>
        <tr>
            <td>
                <label for="everestforms_forms_select"><?php _e('Notification from this forms', $this->plugin_key) ?></label>
            </td>
            <td>
                <?php
                self::forms_select(
