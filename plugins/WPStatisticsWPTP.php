...
class WPStatisticsWPTP extends TelegramBot
{
    ...
    function settings_content()
    {
        ...
        <tr>
            <td>
                <label for="wpstatistics_send_report_notification"><?php _e('Notification for statistical reporting', $this->plugin_key) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="wpstatistics_send_report_notification"
                              name="wpstatistics_send_report_notification" <?php checked($this->get_option('wpstatistics_send_report_notification', 0), 1) ?>> <?php _e('Active', $this->plugin_key) ?>
                </label>
                <?php
                //if (!$WP_Statistics->get_option('stats_report'))
                echo '<br><span class="description">' . __('Requires enable "Statistical reporting" option in the Notification tab in WP Statistics settings.', $this->plugin_key) . '</span>';
                ?>
            </td>
        </tr>
        <?php
